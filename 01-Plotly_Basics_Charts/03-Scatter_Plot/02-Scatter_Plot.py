...
fig_subplots = make_subplots(rows=1, cols=2, subplot_titles=("Department: IT", "Department: HR"))
...
fig_subplots.add_trace(
    go.Scatter(
        ...
        mode="markers",
        marker=dict(
            size=df_it["PerformanceScore"] * 3, color="blue", opacity=0.7  # Scale marker size
        ),
        name="IT",
    ),
    row=1,
...
fig_subplots.add_trace(
    go.Scatter(
        ...
        y=df_hr["Salary"],
        mode="markers",
        marker=dict(size=df_hr["PerformanceScore"] * 3, color="green", opacity=0.7),
        name="HR",
    ),
    row=1,
    col=2,
...
fig_subplots.update_layout(title="Subplots: Comparing IT vs. HR", showlegend=True, height=600)
